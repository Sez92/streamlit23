...
if nav == "Data":
    ...
    elif st.checkbox("Click here to see the correlation between variables"):
        fig,ax = plt.subplots()
        st.title("Heatmap features titanic dataset")
        sns.heatmap(df.corr(), ax = ax)
        st.write(fig)

    elif st.checkbox("Click here to see distribution of variables"):
        st.title("Distribution features titanic dataset")
        fig = sns.pairplot(df, hue = "Survived")
        st.pyplot(fig)

    elif st.checkbox("Click here to see the variables taken into account for model fit"):
...
# using widgets for user interaction: 
    ...
    male_pclass = col6.slider(
        ...
        max_value = 3,
        step = 1,
        key=101 # by using more than 1 slider you have to pass a key (random number) to separate them
        )
